...
function toggleEditMode(isEditMode) {
  ...
  document.getElementById('charCount').style.display = isEditMode ? 'inline-block' : 'none';  
...
document.getElementById('save-button').addEventListener('click', async function(event) {
  ...
  const currentUserData = getLocalStorage();
  const userRef = doc(db, "usersFamily", currentUser.uid);
  ...
  const newDescription = document.getElementById("description").value || currentUserData.description;
  ...
  try {
    ...
    if (newDescription) newUserData.description = newDescription;

    if (newPhotoFile && validateFile(newPhotoFile)) {
      // Usar o uid do usuário como parte do caminho no Storage
      const fileRef = ref(storage, `usersFamilyPhotos/${currentUser.uid}/${newPhotoFile.name}`);
      await uploadBytes(fileRef, newPhotoFile);
      // Obter o URL de download usando a nova sintaxe
      const downloadURL = await getDownloadURL(fileRef);
      // Atualizar a URL da foto de perfil no Firestore
      await updateDoc(userRef, { photoURL: downloadURL });
      newUserData.photoURL = downloadURL;
      // Atualizar a exibição da foto de perfil na página
